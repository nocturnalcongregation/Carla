...
import CarlaInfoDisplay, {
  type PatchbayConnection as AppPatchbayConnection,
...
function App() {
  ...
  const handleManualConnectPorts = async () => {
    if (
      !manualSourcePluginId ||
      !manualSourcePortIndex ||
      !manualTargetPluginId ||
      !manualTargetPortIndex
    ) {
      ...
      <div className="container mx-auto p-4">
        ...
        <div className="space-y-2 mb-4 p-4 border rounded-lg shadow bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2 items-end">
            <div>
              <label htmlFor="src-plug-id" className="text-xs">
                SrcPlgID:
              </label>
              <input
                id="src-plug-id"
                type="text"
                value={manualSourcePluginId}
                onChange={(e) => setManualSourcePluginId(e.target.value)}
                className="input input-bordered input-xs w-full"
              />
            </div>
            <div>
              <label htmlFor="src-port-idx" className="text-xs">
                SrcPrtIdx:
              </label>
              <input
                id="src-port-idx"
                type="text"
                value={manualSourcePortIndex}
                onChange={(e) => setManualSourcePortIndex(e.target.value)}
                className="input input-bordered input-xs w-full"
              />
            </div>
            <div>
              <label htmlFor="tgt-plug-id" className="text-xs">
                TgtPlgID:
              </label>
              <input
                id="tgt-plug-id"
                type="text"
                value={manualTargetPluginId}
                onChange={(e) => setManualTargetPluginId(e.target.value)}
                className="input input-bordered input-xs w-full"
              />
            </div>
            <div>
              <label htmlFor="tgt-port-idx" className="text-xs">
                TgtPrtIdx:
              </label>
              <input
                id="tgt-port-idx"
                type="text"
                value={manualTargetPortIndex}
                onChange={(e) => setManualTargetPortIndex(e.target.value)}
                className="input input-bordered input-xs w-full"
              />
            </div>
            <button
              onClick={handleManualConnectPorts}
        ...
        {lastMessage && (
          <div className="mt-2 p-2 bg-gray-100 rounded text-xs border">
            <p>
              Last OSC: {lastMessage.address} {JSON.stringify(lastMessage.args)}
            </p>
          </div>
        )}
