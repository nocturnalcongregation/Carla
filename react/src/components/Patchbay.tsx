...
import type {
  Patch,
  Connection as PatchbayConnection,
  Port,
  PortType,
} from "../types";
...
const PatchNode: React.FC<{ data: Patch; selected?: boolean }> = ({
  data,
  selected,
}) => {
  const nodeClasses = `
    border-2 ${
      selected ? "border-blue-600 shadow-lg" : "border-gray-400 shadow-md"
    }
  ...
  const headerClasses =
    "font-bold text-sm mb-3 text-center text-gray-800 border-b border-gray-200 pb-2";
  ...
  const portNameClasses =
    "mx-1.5 whitespace-nowrap overflow-hidden overflow-ellipsis";

  const getHandleClasses = (portType: PortType): string => {
    return `w-3 h-3 rounded-full border border-gray-700 ${
      PORT_TYPE_BG_COLORS[portType] || "bg-gray-300"
    }`;
  };
            ...
            .map((port) => (
              <div key={port.id} className={portItemClasses}>
                <Handle
                  ...
                  isConnectable={true}
                />
                <span className={portNameClasses} title={port.name}>
                  {port.name}
                </span>
              </div>
            ))}
    ...
    setRfEdges(
      connections.map((conn: PatchbayConnection): Edge => {
        ...
        if (sourcePort) {
          ...
          else if (sourcePort.type === PortType.Midi)
    ...
    async (deletedEdges) => {
      for (const edge of deletedEdges) {
        ...
        if (connectionToDelete) {
          ...
          console.warn(
            `Could not find connection data for edge ID ${edge.id} to delete.`
          );
        }
      }
    },
  ...
  if (isLoading && !patches.length && !connections.length && !isConnected) {
    return (
      <div className="p-4 text-center">
        Connecting to Carla or loading patchbay...
      </div>
    );
  }
    ...
    <div className="h-screen w-full flex flex-col bg-gray-50">
      <div className="p-3 border-b border-gray-300 bg-gray-100 shadow">
        <h3 className="text-lg font-semibold text-gray-700">
          Carla Patchbay Control (React PWA)
        </h3>
        {!isConnected ? (
          ...
          <div className="mt-2 flex items-center space-x-2">
            <p className="text-sm text-green-600">
              Status: Connected to {host}:{tcpPort}
            </p>
            <button
