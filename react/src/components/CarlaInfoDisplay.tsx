...
export interface ProgramCounts { programs: number; midiPrograms: number; }
...
export interface PluginInfo {
  id: number; type?: string; category?: string; hints?: number; uniqueId?: number; // type changed to string to match useOsc
...
export interface EngineInfo { [key: string]: any; } // This is not passed as a prop anymore
export interface PatchbayConnection { // This should align with useOsc's PatchbayConnection
  id: string; sourcePluginId: number; sourcePortIndex: number;
  targetPluginId: number; targetPortIndex: number;
  sourcePortName?: string; targetPortName?: string; // Optional names
}
...
interface CarlaInfoDisplayProps {
  ...
  setParameterValueRequest: (vars: { pluginId: number; paramId: number; value: number }) => Promise<any>;
  fetchParametersForPlugin: (pluginId: number) => void;
  getStoredParametersForPlugin: (pluginId: number) => PluginParameter[];
}

const ParameterInput: React.FC<{ pluginId: number, param: PluginParameter, setParameterValueRequest: CarlaInfoDisplayProps['setParameterValueRequest'] }> =
  ...
  const handleBlur = async () => {
    ...
    if (!isNaN(numValue) && numValue !== param.value) {
      try {
        await setParameterValueRequest({ pluginId, paramId: param.id, value: numValue });
      } catch (error) {
...
const PluginParamsDisplay: React.FC<{
  ...
  if (parameters.length === 0) return <p className='text-xs text-gray-500'>No parameters available or loaded. Click plugin name to load.</p>;
...
const PatchbayConnectionsDisplay: React.FC<{
  ...
  const handleDisconnect = async (connectionId: string) => {
    if (window.confirm(`Are you sure you want to disconnect connection ${connectionId}?`)) {
      try {
      ...
      <ul className='space-y-1 text-xs'>
        {connections.map(conn => (
          <li key={conn.id} className='flex justify-between items-center p-1 bg-gray-100 rounded'>
            ...
            <button onClick={() => handleDisconnect(conn.id)} className='btn btn-warning btn-xs'>X</button>
          </li>
        ))}
      </ul>
    </div>
...
export const CarlaInfoDisplay: React.FC<CarlaInfoDisplayProps> = ({
  ...
  const [currentDisplayParams, setCurrentDisplayParams] = useState<PluginParameter[]>([]);
  const queryClient = useQueryClient();

  const togglePluginDetails = useCallback((pluginId: number) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = queryClient.getQueryCache().subscribe(event => {
      if (event.query &&
          ...
          String(event.query.queryKey[1]) === String(expandedPluginId)) { // Check if it's for the currently expanded plugin
  ...
  if (isLoading && !plugins.length) return <div className="text-center p-4">Loading plugin info...</div>;
  ...
  return (
    <div className='mt-4 p-4 border rounded-md bg-gray-50'>
      ...
      {plugins && plugins.length > 0 ? (
            ...
            {plugins.map((plugin) => (
              <li key={plugin.id} className='text-sm border p-3 rounded bg-white shadow-sm'>
                <div className='flex justify-between items-center'>
                  ...
                  <button onClick={() => handleRemovePlugin(plugin.id)} className='btn btn-error btn-xs'>Remove</button>
                </div>
        ...
        !isLoading && <p className='text-xs text-gray-500'>No plugins loaded. Connect to Carla and refresh.</p>
      )}
      <PatchbayConnectionsDisplay connections={connections} disconnectPortsRequest={disconnectPortsRequest} />
    </div>
  );
};
