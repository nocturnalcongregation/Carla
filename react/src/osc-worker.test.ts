...
const workerMethods = {
  connect: async (host: string, tcpPort: number, udpPort: number) => {
    console.log(
      `[Worker] Attempting to connect to ${host}, TCP: ${tcpPort}, UDP (conceptual): ${udpPort}`
    );
    ...
    oscOverTCP.on("*", (message: OSC.Message) => {
      console.log(
        "[Worker] OSC TCP (WebSocket) Message Received:",
        message.address,
        message.args
      );
    ...
    try {
      ...
      console.error(
        "[Worker] Failed to initiate OSC TCP (WebSocket) connection:",
        error
      );
      statusCallback?.(
        `OSC TCP Connection Failed: ${(error as Error).message}`
      );
      return;
    }

    console.log(
      "[Worker] UDP communication is conceptual. All messages will be sent via the primary TCP/WebSocket connection."
    );
  ...
  sendMessage: (address: string, ...args: any[]) => {
    if (oscOverTCP && oscOverTCP.status() === OSC.STATUS.IS_OPEN) {
      ...
      try {
        ...
        console.error(
          `[Worker] Error sending OSC message ${address} via TCP WebSocket:`,
          error
        );
      ...
      console.warn(
        "[Worker] OSC TCP (WebSocket) connection not open. Message not sent:",
        address,
        args
      );
  ...
  addPlugin: (
    ...
    if (oscOverTCP && oscOverTCP.status() === OSC.STATUS.IS_OPEN) {
      ...
      console.log(
        `[Worker] Sent /ctrl/add_plugin (mid: ${messageId}): ${type} - ${name}`
      );
      return messageId;
    }
  ...
  removePlugin: (pluginId: number) => {
    if (oscOverTCP && oscOverTCP.status() === OSC.STATUS.IS_OPEN) {
      const messageId = nextMessageId++;
      oscOverTCP.send(
        new OSC.Message("/ctrl/remove_plugin", messageId, pluginId)
      );
      console.log(
        `[Worker] Sent /ctrl/remove_plugin (mid: ${messageId}): pluginId ${pluginId}`
      );
      return messageId;
    }
  ...
  connectPorts: (
    ...
    if (oscOverTCP && oscOverTCP.status() === OSC.STATUS.IS_OPEN) {
      ...
      console.log(
        `[Worker] Sent /ctrl/patchbay_connect (mid: ${messageId}): ${sourcePluginId}:${sourcePortIndex} -> ${targetPluginId}:${targetPortIndex}`
      );
      return messageId;
    }
  ...
  disconnectPorts: (
    ...
    if (oscOverTCP && oscOverTCP.status() === OSC.STATUS.IS_OPEN) {
      ...
      console.log(
        `[Worker] Sent /ctrl/patchbay_disconnect (mid: ${messageId}): ${sourcePluginId}:${sourcePortIndex} -> ${targetPluginId}:${targetPortIndex}`
      );
      return messageId;
    }
  ...
  sendDataMessage: (address: string, ...args: any[]) => {
    if (oscOverTCP && oscOverTCP.status() === OSC.STATUS.IS_OPEN) {
      console.log(
        `[Worker] Sending Data OSC (via TCP WebSocket): ${address}`,
        args
      );
      try {
        oscOverTCP.send(new OSC.Message(address, ...args));
        ...
        console.error(
          `[Worker] Error sending Data OSC message ${address} via TCP WebSocket:`,
          error
        );
        statusCallback?.(
          `Error sending data message: ${(error as Error).message}`
        );
      }
    } else {
      console.warn(
        "[Worker] OSC TCP (WebSocket) connection not open for data message. Message not sent:",
        address,
        args
      );
      statusCallback?.(
        "Error: OSC TCP not connected. Cannot send data message."
      );
    }
  },
