...
import type { Patch, Connection, Port, PatchbayState, PortType, PortDirection } from '../types'; // Path to your type definitions
...
export const usePatchbay = () => {
  ...
  const handleOscMessage = useCallback((message: ReceivedOscMessage) => {
    console.log('[usePatchbay] OSC Message Received:', message.address, message.args);

    // Update patches based on /ctrl/info, /ctrl/ports, etc.
    if (message.address === '/ctrl/info') {
      const [id, type, category, _hints, _uniqueId, _optsAvail, _optsEnabled, name, _filename, _iconName, _realName, label] = message.args;
      queryClient.setQueryData<Patch[]>(PATCHES_QUERY_KEY, (oldPatches = []) => {
        const existingIndex = oldPatches.findIndex(p => p.id === String(id));
        const newPatchData: Partial<Patch> = { name, label: label || name, type, category };
        if (existingIndex !== -1) {
          const updatedPatches = [...oldPatches];
          updatedPatches[existingIndex] = { ...updatedPatches[existingIndex], ...newPatchData };
          return updatedPatches;
        }
        // Placeholder for ports, position, data - these might come from other messages or need defaults
        return [...oldPatches, { id: String(id), ports: [], ...newPatchData } as Patch];
      });
    } else if (message.address === '/ctrl/ports') {
      const [pluginId, audioIns, audioOuts, midiIns, midiOuts, cvIns, cvOuts] = message.args;
      queryClient.setQueryData<Patch[]>(PATCHES_QUERY_KEY, (oldPatches = []) => {
        const patchIndex = oldPatches.findIndex(p => p.id === String(pluginId));
        if (patchIndex === -1) return oldPatches;

        const newPorts: Port[] = [];
        // Helper to create ports
        const createPorts = (count: number, type: PortType, direction: PortDirection, offset: number) => {
          for (let i = 0; i < count; i++) {
            newPorts.push({
              id: `${pluginId}-${direction === PortDirection.Input ? 'in' : 'out'}-${type}-${i + offset}`,
              name: `${type.toUpperCase()} ${direction === PortDirection.Input ? 'In' : 'Out'} ${i + 1 + offset}`,
              type,
              direction,
        ...
        let portIdxOffset = 0;
        createPorts(audioIns, PortType.Audio, PortDirection.Input, portIdxOffset); portIdxOffset += audioIns;
        createPorts(cvIns, PortType.Cv, PortDirection.Input, portIdxOffset); portIdxOffset += cvIns;
        createPorts(midiIns, PortType.Midi, PortDirection.Input, portIdxOffset); portIdxOffset += midiIns;

        portIdxOffset = 0; // Reset for outputs
        createPorts(audioOuts, PortType.Audio, PortDirection.Output, portIdxOffset); portIdxOffset += audioOuts;
        createPorts(cvOuts, PortType.Cv, PortDirection.Output, portIdxOffset); portIdxOffset += cvOuts;
        createPorts(midiOuts, PortType.Midi, PortDirection.Output, portIdxOffset);

        const updatedPatches = [...oldPatches];
        updatedPatches[patchIndex] = { ...updatedPatches[patchIndex], ports: newPorts };
        return updatedPatches;
      });
    } else if (message.address === '/ctrl/cb') { // Callback messages
      const [action, value1, value2, value3, _valueFloat, valueStr] = message.args;
      ...
      if (action === 1 /* PLUGIN_MOVE */) {
        ...
        const y = value3;
        queryClient.setQueryData<Patch[]>(PATCHES_QUERY_KEY, (oldPatches = []) =>
          oldPatches.map(p => p.id === patchId ? { ...p, position: { x, y } } : p)
        );
      }
      // Action 2: PLUGIN_REMOVE
        ...
        queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, (oldConnections = []) =>
          oldConnections.filter(c => c.source !== removedPatchId && c.target !== removedPatchId)
        );
      }
      // Action 3: PORTS_CONNECT
        ...
        const connId = valueStr || `${value1}-${value2}_${value3}-${message.args[3]}`; // args[3] is dst_port_idx
        ...
        queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, (oldConnections = []) => {
          if (oldConnections.find(c => c.id === connId)) return oldConnections; // Already exists
          // We need to map indices to actual port IDs (handles)
          const patches = queryClient.getQueryData<Patch[]>(PATCHES_QUERY_KEY) || [];
          const sourcePatch = patches.find(p => p.id === sourcePatchId);
          const targetPatch = patches.find(p => p.id === targetPatchId);

          const sourcePort = sourcePatch?.ports.filter(p=>p.direction === PortDirection.Output)[sourcePortIndex];
          const targetPort = targetPatch?.ports.filter(p=>p.direction === PortDirection.Input)[targetPortIndex];
          ...
          console.warn(`[usePatchbay] Could not find ports for connection: ${connId}`);
          return oldConnections;
        });
      }
      // Action 4: PORTS_DISCONNECT
        ...
        const connId = valueStr || `${value1}-${value2}_${value3}-${message.args[3]}`;
        queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, (oldConnections = []) =>
          oldConnections.filter(c => c.id !== connId)
        );
      }
    } else if (message.address === '/ctrl/patchbay_info') { // From /ctrl/get_patchbay_canvas_info
        // /ctrl/patchbay_info, client_id, src_id, src_port_idx, dst_id, dst_port_idx
        const [_clientId, sourcePluginId, sourcePortIndex, targetPluginId, targetPortIndex] = message.args;
        const connId = `${sourcePluginId}-${sourcePortIndex}_${targetPluginId}-${targetPortIndex}`;

        queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, (oldConnections = []) => {
          if (oldConnections.find(c => c.id === connId)) return oldConnections;

          const patches = queryClient.getQueryData<Patch[]>(PATCHES_QUERY_KEY) || [];
          const sourcePatch = patches.find(p => p.id === String(sourcePluginId));
          const targetPatch = patches.find(p => p.id === String(targetPluginId));
          ...
          console.warn(`[usePatchbay] Could not find ports for /ctrl/patchbay_info connection: ${connId}`);
          return oldConnections;
        });
      ...
      if (mutationInfo) {
        if (errorStr && errorStr.length > 0) {
          console.error(`[usePatchbay] Mutation (id: ${messageId}, type: ${mutationInfo.type}) failed: ${errorStr}`);
          ...
          console.log(`[usePatchbay] Mutation (id: ${messageId}, type: ${mutationInfo.type}) successful.`);
        ...
        console.warn(`[usePatchbay] Received /ctrl/resp for unknown messageId: ${messageId}`);
      }
    }
  ...
  useEffect(() => {
    const statusUnsub = oscWorker.onStatusChange(Comlink.proxy((status: string) => {
    ...
    const messageUnsub = oscWorker.onMessage(Comlink.proxy(handleOscMessage as (msg: OSC.Message) => void));
  ...
  const { data: connections = [], isLoading: isLoadingConnections } = useQuery<Connection[]>({
    ...
    queryFn: async () => {
      ...
      return queryClient.getQueryData<Connection[]>(CONNECTIONS_QUERY_KEY) || [];
    },
    initialData: [],
    staleTime: Infinity,
  });
  ...
  const addPluginMutation = useMutation({
    mutationFn: async (variables: { type: string; name: string; label: string; x?: number; y?: number }) => {
      const { type, name, label, x = 50, y = 50 } = variables;
      const messageId = await oscWorker.addPlugin(type, name, label, x, y);
      if (messageId === -1) throw new Error('OSC worker not connected or failed to send message.');
      pendingMutations.set(messageId, { type: 'addPlugin', optimisticUpdateApplied: false }); // No easy optimistic update for new plugin ID
      return { messageId };
    },
  ...
  const removePluginMutation = useMutation({
    mutationFn: async (pluginId: string) => {
      ...
      // Optimistic update
      const previousPatches = queryClient.getQueryData<Patch[]>(PATCHES_QUERY_KEY) || [];
      const previousConnections = queryClient.getQueryData<Connection[]>(CONNECTIONS_QUERY_KEY) || [];
      queryClient.setQueryData<Patch[]>(PATCHES_QUERY_KEY, old => old?.filter(p => p.id !== pluginId) || []);
      queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, old => old?.filter(c => c.source !== pluginId && c.target !== pluginId) || []);
    ...
    onError: (error, pluginId, context) => {
      ...
      if (mutationInfo?.optimisticUpdateApplied && mutationInfo.data) {
        queryClient.setQueryData<Patch[]>(PATCHES_QUERY_KEY, mutationInfo.data.previousPatches);
        queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, mutationInfo.data.previousConnections);
      }
    },
  });

  const connectPortsMutation = useMutation({
    mutationFn: async (variables: { sourcePatchId: string; sourcePortId: string; targetPatchId: string; targetPortId: string }) => {
      const { sourcePatchId, sourcePortId, targetPatchId, targetPortId } = variables;
      // We need to convert port IDs (handles) back to indices for Carla if worker expects indices
      const patchesData = queryClient.getQueryData<Patch[]>(PATCHES_QUERY_KEY) || [];
      const sourcePatch = patchesData.find(p => p.id === sourcePatchId);
      const targetPatch = patchesData.find(p => p.id === targetPatchId);

      const sourcePortIndex = sourcePatch?.ports.filter(p=>p.direction === PortDirection.Output).findIndex(p => p.id === sourcePortId);
      const targetPortIndex = targetPatch?.ports.filter(p=>p.direction === PortDirection.Input).findIndex(p => p.id === targetPortId);

      if (sourcePortIndex === undefined || sourcePortIndex === -1 || targetPortIndex === undefined || targetPortIndex === -1) {
        throw new Error('Could not find one or both ports by ID to determine index.');
      }

      const messageId = await oscWorker.connectPorts(parseInt(sourcePatchId,10), sourcePortIndex, parseInt(targetPatchId,10), targetPortIndex);
      if (messageId === -1) throw new Error('OSC worker not connected or failed to send message.');
      ...
      const previousConnections = queryClient.getQueryData<Connection[]>(CONNECTIONS_QUERY_KEY) || [];
      queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, old => [...(old || []), newConnection]);
    ...
    onSuccess: () => { console.log('Connect ports mutation sent.'); },
    onError: (error, _variables, context) => {
      console.error('Error connecting ports:', error);
      const mutationInfo = pendingMutations.get(context?.messageId as number);
      if (mutationInfo?.optimisticUpdateApplied && mutationInfo.data) {
        // Revert by removing the optimistically added connection
        queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY,
          mutationInfo.data.previousConnections.filter((c: Connection) => c.id !== mutationInfo.data.newConnectionId)
        );
      }
    },
  });
  ...
  const disconnectPortsMutation = useMutation({
    mutationFn: async (variables: { connection: Connection }) => {
      ...
      const sourcePortIndex = sourcePatch?.ports.filter(p=>p.direction === PortDirection.Output).findIndex(p => p.id === connection.sourceHandle);
      const targetPortIndex = targetPatch?.ports.filter(p=>p.direction === PortDirection.Input).findIndex(p => p.id === connection.targetHandle);

      if (sourcePortIndex === undefined || sourcePortIndex === -1 || targetPortIndex === undefined || targetPortIndex === -1) {
        throw new Error('Could not find one or both ports by ID to determine index for disconnection.');
      }

      const messageId = await oscWorker.disconnectPorts(parseInt(connection.source,10), sourcePortIndex, parseInt(connection.target,10), targetPortIndex);
      if (messageId === -1) throw new Error('OSC worker not connected or failed to send message.');

      // Optimistic update
      const previousConnections = queryClient.getQueryData<Connection[]>(CONNECTIONS_QUERY_KEY) || [];
      queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, old => old?.filter(c => c.id !== connection.id) || []);
    ...
    onSuccess: () => { console.log('Disconnect ports mutation sent.'); },
    onError: (error, _variables, context) => {
      console.error('Error disconnecting ports:', error);
      const mutationInfo = pendingMutations.get(context?.messageId as number);
      if (mutationInfo?.optimisticUpdateApplied && mutationInfo.data) {
         // Revert by adding back the connection if it was part of previousConnections
        const previousConnection = mutationInfo.data.previousConnections.find((c: Connection) => c.id === mutationInfo.data.removedConnectionId);
        if (previousConnection) {
            queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, old => [...(old || []), previousConnection]);
        } else { // Or just set to previous state if simpler
            queryClient.setQueryData<Connection[]>(CONNECTIONS_QUERY_KEY, mutationInfo.data.previousConnections);
        }
      }
    },
