...
import type {
  ...
  PluginParameter,
  PatchbayConnection,
} from "../components/CarlaInfoDisplay";
...
export const QK_PLUGIN_PARAMETERS = (pluginId: number | string): QueryKey => [
  "pluginParameters",
  String(pluginId),
];
...
export const useOsc = () => {
  ...
  const [lastMessage, setLastMessage] = useState<ReceivedOscMessage | null>(
    null
  );

  const updatePluginParametersInCache = useCallback(
    (pluginId: number, paramId: number, updates: Partial<PluginParameter>) => {
      queryClient.setQueryData<PluginParameter[]>(
        QK_PLUGIN_PARAMETERS(pluginId),
        (oldParams = []) => {
  ...
  const handleOscMessage = useCallback(
    (msg: ReceivedOscMessage) => {
      ...
      if (msg.address === "/ctrl/info") {
        ...
        const newPluginData: PluginInfo = {
          id: pluginId,
          type,
          category,
          hints,
          ...
          copyright,
          ports: {} as PortCounts,
          programCounts: {} as ProgramCounts,
          internalParams: {} as InternalParams,
        };
        queryClient.setQueryData<PluginInfo[]>(
          QK_PLUGIN_LIST,
          (oldData = []) => {
        ...
        queryClient.setQueryData<PluginInfo[]>(QK_PLUGIN_LIST, (oldData = []) =>
          oldData.map((p) =>
            p.id === pluginId ? { ...p, ports: portData } : p
          )
        );
          ...
          (old) =>
            old?.map((p) =>
              p.id === pluginId
                ? {
                    ...p,
                    programCounts: { programs: pcount, midiPrograms: mpcount },
                  }
                : p
            ) || []
        );
              ...
              p.id === pluginId
                ? {
                    ...
                    internalParams: {
                      ...
                      volume,
                      balanceLeft: balLeft,
                      balanceRight: balRight,
                      panning: pan,
                      ctrlChannel: ctrlChan,
                    },
                  }
                : p
            ) || []
        );
      } else if (msg.address === "/ctrl/paramInfo") {
        const [pluginId, paramId, name, unit, comment, groupName] = msg.args;
        updatePluginParametersInCache(pluginId, paramId, {
          name,
          unit,
          comment,
          groupName,
        });
      } else if (msg.address === "/ctrl/paramData") {
        const [
          pluginId,
          paramId,
          type,
      ...
      } else if (msg.address === "/ctrl/paramRanges") {
        const [
          pluginId,
          paramId,
          defaultValue,
      ...
      } else if (msg.address === "/ctrl/param") {
        const [pluginId, paramId, value] = msg.args;
        updatePluginParametersInCache(pluginId, paramId, { value });
        ...
        if (action === 2 /* PLUGIN_REMOVE */) {
          ...
          queryClient.setQueryData<PluginInfo[]>(QK_PLUGIN_LIST, (old = []) =>
            old.filter((p) => p.id !== removedPluginId)
          );
          queryClient.removeQueries({
            queryKey: QK_PLUGIN_PARAMETERS(removedPluginId),
          });
          queryClient.setQueryData<PatchbayConnection[]>(
            QK_PATCHBAY_CONNECTIONS,
            (oldConns = []) =>
              oldConns.filter(
                (c) =>
                  c.sourcePluginId !== removedPluginId &&
                  c.targetPluginId !== removedPluginId
              )
          );
        } else if (action === 3 /* PORTS_CONNECT */) {
          const connId =
            valueStr || `${value1}-${value2}_${value3}-${msg.args[3]}`;
          const newConn: PatchbayConnection = {
            id: connId,
            sourcePluginId: value1,
            sourcePortIndex: value2,
            targetPluginId: value3,
            targetPortIndex: msg.args[3],
          };
          queryClient.setQueryData<PatchbayConnection[]>(
            QK_PATCHBAY_CONNECTIONS,
            (old = []) => {
  ...
  const pluginListQuery = useQuery<PluginInfo[], Error>({
    ...
    queryFn: async () => {
      if (connectionStatus.toLowerCase().includes("connected")) {
        await oscWorker.requestPluginList();
      }
      return queryClient.getQueryData<PluginInfo[]>(QK_PLUGIN_LIST) || [];
    },
    enabled: connectionStatus.toLowerCase().includes("connected"),
    staleTime: 60000,
  ...
  const patchbayConnectionsQuery = useQuery<PatchbayConnection[], Error>({
    ...
    queryFn: async () => {
      return (
        queryClient.getQueryData<PatchbayConnection[]>(
          QK_PATCHBAY_CONNECTIONS
        ) || []
      );
    },
  ...
  interface OptimisticContext<TData = any> {
    previousData?: TData;
    messageId?: number;
  }
  ...
  const addPluginMutation = useMutation<
    { messageId: number },
    Error,
    { type: string; name: string; label: string; x?: number; y?: number },
    OptimisticContext<PluginInfo[]>
  >({
    mutationFn: async (variables) => {
      ...
      if (messageId === -1)
        throw new Error("Failed to send addPlugin message.");
      return { messageId };
    },
    onMutate: async (_newPluginVars) => {
      await queryClient.cancelQueries({ queryKey: QK_PLUGIN_LIST });
      const previousPluginList =
        queryClient.getQueryData<PluginInfo[]>(QK_PLUGIN_LIST) || [];
      return { previousData: previousPluginList };
    },
    onError: (_err, _newPlugin, context) => {
  ...
  >({
    mutationFn: async ({ pluginId }) => {
      ...
      if (messageId === -1)
        throw new Error("Failed to send removePlugin message.");
      return { messageId };
    },
  ...
  const setParameterValueMutation = useMutation<
    { success: boolean },
    Error,
    { pluginId: number; paramId: number; value: number },
    OptimisticContext<PluginParameter[]>
  >({
    ...
    onMutate: async (variables) => {
      ...
      const previousParams =
        queryClient.getQueryData<PluginParameter[]>(queryKey) || [];
      const optimisticParams = previousParams.map((p) =>
        p.id === paramId ? { ...p, value } : p
      );
      if (previousParams.find((p) => p.id === paramId))
        queryClient.setQueryData<PluginParameter[]>(queryKey, optimisticParams);
      return { previousData: previousParams };
    },
    onError: (_err, variables, context) => {
      if (context?.previousData)
        queryClient.setQueryData<PluginParameter[]>(
          QK_PLUGIN_PARAMETERS(variables.pluginId),
          context.previousData
        );
    },
    onSettled: (_data, _error, variables) =>
      queryClient.invalidateQueries({
        queryKey: QK_PLUGIN_PARAMETERS(variables.pluginId),
      }),
  });
  ...
  >({
    mutationFn: async (vars) => {
      const messageId = await oscWorker.connectPorts(
        vars.sourcePluginId,
        vars.sourcePortIndex,
        vars.targetPluginId,
        vars.targetPortIndex
      );
      if (messageId === -1)
        throw new Error("Failed to send connectPorts message.");
      return { messageId };
    },
    onSettled: () =>
      queryClient.invalidateQueries({ queryKey: QK_PATCHBAY_CONNECTIONS }),
  ...
  const disconnectPortsMutation = useMutation<
    ...
    Error,
    { connectionId: string },
    OptimisticContext<PatchbayConnection[]>
  >({
    mutationFn: async ({ connectionId }) => {
      ...
      if (!connToDisconnect) {
        throw new Error(
          `Connection with ID ${connectionId} not found in cache.`
        );
      }
      ...
      if (messageId === -1)
        throw new Error("Failed to send disconnectPorts message.");
      return { messageId };
    },
  ...
  return {
    ...
    fetchPluginParameters: (pluginId: number) => {
      if (connectionStatus.toLowerCase().includes("connected")) {
        oscWorker.sendMessage(
          `/${OSC_CARLA_TARGET_NAME}/${pluginId}/get_all_parameter_info`
        );
        oscWorker.sendMessage(
          `/${OSC_CARLA_TARGET_NAME}/${pluginId}/get_all_parameter_data`
        );
        oscWorker.sendMessage(
          `/${OSC_CARLA_TARGET_NAME}/${pluginId}/get_all_parameter_ranges`
        );
      }
    },
    getPluginParametersFromCache: (pluginId: number | string) => {
      return (
        queryClient.getQueryData<PluginParameter[]>(
          QK_PLUGIN_PARAMETERS(pluginId)
        ) || []
      );
    },
